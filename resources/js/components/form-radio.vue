  ...
  <label :class="{ block: !flex, 'flex flex-col md:flex-row': flex }">
    ...
    <div class="flex flex-col w-full space-y-2 md:pt-3" :class="{ 'md:w-3/4': flex }">
      <label v-for="(option, value) in options" v-bind:key="value" class="inline-flex items-center">
        <input type="radio" class="bg-[#eff2f5] text-[#009ef7] w-5 h-5 border-0 focus:ring-0" v-bind="$attrs" :name="name" :value="value" :checked="value == $attrs.value" v-on="{ ...$listeners, input: (event) => $emit('input', event.target.value) }" /><span class="ml-2 text-sm">{{ option }}</span>
      </label>
