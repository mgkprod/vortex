<template>
  <label :class="{ block: !flex, 'flex flex-col md:flex-row': flex }">
    ...
    <div class="flex flex-col w-full space-y-2 md:pt-3" :class="{ 'md:w-3/4': flex }">
      <label v-for="(option, val) in options" v-bind:key="val" class="inline-flex items-center">
        <input type="checkbox" class="bg-[#eff2f5] text-[#009ef7] w-5 h-5 border-0 focus:ring-0" v-bind="$attrs" :name="name" :value="val" v-model="value" /><span class="ml-2 text-sm">{{ option }}</span>
      </label>
